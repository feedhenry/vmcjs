...
//   ...
// ]
VMC.prototype.appLogs = function(name, options, callback) {
  if (typeof(options) == 'function') {
    callback = options;
    options = undefined;
  }
  var path = '/apps/' + name + '/instances/0/files/logs';
  this.get(path, options, function(err, data) {
    var i;
    var ret = [];
...
VMC.prototype.appLog = function(name, fileName, callback) {
  if(!fileName) {  // if no logfile specified then get latest
    return this.appDefaultLog(name, callback);
  } else {
    var path = '/apps/' + name + '/instances/0/files/logs/' + fileName;
...
VMC.prototype.appDefaultLog = function(appName, callback) {
  var self = this;
  self.appLogs(appName, function(err, logs) {
    var fileContents = {};
    if(err) return callback(err);
    async.forEachSeries(logs, function (log, cb) {
      self.appLog(appName, log.name, function (err, contents) {
